...
if [ -z "$JDK" -a -s "$HOME/.IntelliJIdea2019.1/config/idea.jdk" ]; then
  USER_JRE=`"$CAT" $HOME/.IntelliJIdea2019.1/config/idea.jdk`
  if [ ! -d "$USER_JRE" ]; then
...
elif [ -r "$HOME/.IntelliJIdea2019.1/config/idea$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.IntelliJIdea2019.1/config/idea$BITS.vmoptions"
